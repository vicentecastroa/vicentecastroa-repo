...
class Worker(threading.Thread):
    ...
    def functions(func_name):
        ...
        def open(star_name):
            ...
            # __builtins__.open
            with __builtins__.open("{}.txt".format(star_name), "r") as file:
                loaded_stars["{}".format(star_name)] = file.readlines
        ...
        def mean(star_name):
            ...
            with Worker.lock:
                if star_name not in loaded_stars.keys():
                    open(star_name)
                data_star = loaded_stars["{}".format(star_name)]
                ans = sum(map(lambda l: float(l), data_star))/len(data_star)
                return ans
        ...
        def var(star_name):
            ...
            with Worker.lock:
                data_star = loaded_stars["{}".format(star_name)]
                n = len(data_star)
                suma = sum(map(lambda l: (float(l) - prom)**2, data_star))
                return suma/(n-1)
    ...
    def run(self):
        ...
        output_list.append((self.getName(), self.command, output, ""))
